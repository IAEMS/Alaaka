...
$phoneLinkAttr = '';
if (!\Auth::check()) {
	if (config('settings.guests_can_apply_jobs') != '1') {
										...
										<ul>
											...
											<li>
												<p class="no-margin">
													...
													@if (!empty($post->company_id))
														<a href="{!! lurl(trans('routes.v-search-company', ['countryCode' => $country->get('icode'), 'id' => $post->company_id])) !!}">
															{{ $post->company_name }}
														</a>
													@else
														{{ $post->company_name }}
											...
											<li>
												...
												<p class="no-margin">
													...
													<a href="{{ lurl(trans('routes.v-search', ['countryCode' => $country->get('icode')])) . '?type[]=' . $post->post_type_id }}">
														{{ $postType->name }}
													</a>
												</p>
												@endif
									...
									<div class="ads-action">
										<ul class="list-border">
											@if (isset($post->company) and !empty($post->company))
												<li>
													<a href="{{ lurl(trans('routes.v-search-company', ['countryCode' => $country->get('icode'), 'id' => $post->company->id])) }}">
...
@section('modal_message')
	@if (\Auth::check() or config('settings.guests_can_apply_jobs')=='1')

...
{
    return starts_with(getDomain(), config('larapen.core.demoDomain')) &&
...
{
	if (!str_contains(\Illuminate\Support\Facades\Route::currentRouteAction(), 'Post\DetailsController')) {
		return null;
	}
 ...
 * @return mixed|string
 */
function str_clean_lite($string)
{
...
function getMetaTag($tag, $page)
{
    ...
    if (in_array($tag, ['title', 'description'])) {
        return config('app.name') . ' - ' . config('settings.app_slogan');
    }

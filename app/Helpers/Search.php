...
class Search
{
    ...
    public function __construct($preSearch = [])
    {
		...
		self::$maxDistance = config('larapen.core.search.distance.max', self::$maxDistance);
		self::$distance = config('larapen.core.search.distance.default', self::$distance);
        ...
        if (Input::filled('distance') and is_numeric(Input::get('distance')) and Input::get('distance') > 0) {
            ...
            if (Input::get('distance') > self::$maxDistance) {
    ...
    public function fechAll()
    {
        ...
        if (Input::has('l') && !empty($this->city)) {

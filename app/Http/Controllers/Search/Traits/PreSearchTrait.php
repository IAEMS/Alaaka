...
trait PreSearchTrait
{
    ...
    public function getCategory($catId, $subCatId = null)
    {
        ...
        if (!empty($subCatId)) {
            ...
            // Get SubCategory
            $this->subCat = Category::find($subCatId);
    ...
    public function getCity($cityId = null, $location = null)
    {
        ...
        if (!empty($location)) {
            ...
            if (str_contains($location, t('area:'))) {
                ...
                $url = qsurl($fullUrlNoParams, array_merge(Input::except(['l', 'location']), ['d' => config('country.code'), 'r' => $adminName]));
    ...
    {
        if (empty($adminName) || Input::filled('l')) {
            return null;
        }
        ...
        else
        {
            ...
            $fullUrl = url(Request::getRequestUri());
            $fullUrlNoParams = head(explode('?', $fullUrl));
            $url = qsurl($fullUrlNoParams, array_merge(Input::except(['r']), ['l' => $this->city->id, 'location' => $adminName]));
